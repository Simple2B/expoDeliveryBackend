...
@restaurant_router.get("/panels", status_code=status.HTTP_200_OK, response_model=s.PanelRestaurantList)
def get_panels(
    ...
    return s.PanelRestaurantList(panels=[s.PanelRestaurant.model_validate(restaurant) for restaurant in restaurants])


@restaurant_router.get("/{restaurant_id}", status_code=status.HTTP_200_OK, response_model=s.Restaurant)
def get_restaurant(
    restaurant_id: int,
    db: Session = Depends(get_db),
):
